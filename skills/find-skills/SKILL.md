---
name: find-skills
description: Use when the user asks "how do I do X", "find a skill for X", "is there a skill that can...", wants to search for tools/templates/workflows, or expresses interest in extending agent capabilities with an installable skill.
---
...
## ⚙️ DSH Environment Note (本机 DSH 环境)

本机通过 DSH 沙箱执行命令时，npm 无法写入系统默认缓存目录（EPERM）。**运行任何 `npx skills` 命令前，必须先把 npm 缓存指到工作区：**

```powershell
$env:npm_config_cache = "C:\Users\lxx10\Desktop\deepseek\.npm-cache"
```

然后正常执行 `npx skills ...`（DSH 的 pwsh 工具本身就是 PowerShell，无需再套 `powershell -Command` 外壳，但套了也无害）。首次运行会下载 CLI 包，需要一点时间。
